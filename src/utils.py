...
molType = TypeVar("molType", Chem.rdchem.Mol)
fngrprntType = TypeVar("fngrprntType", cDataStructs.ExplicitBitVect)
...
def get_mols(df: pd.DataFrame, column: str) -> List[molType]:
    """
...
def get_fingerprints(
    ...
    if as_numpy:
        fingerprints = np.array([fingerprnt_gen.GetFingerprintsAsNumpy(x) for x in mols])
    else:
